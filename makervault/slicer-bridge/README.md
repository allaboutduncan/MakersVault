...
This helper registers the custom protocol `makersvault-slicer://` and launches
a slicer with the downloaded model. It is required for the "Open in Slicer"
button to work across Windows and Linux clients.
...
Download the latest Slicer Bridge binary for your OS from the
[MakerVault releases](https://github.com/VincentCinque/MakersVault/releases/latest)
page and run it once. The installer registers the `makersvault-slicer://` protocol
handler and copies the binary into a stable location:
...
- If a slicer is not found, the file opens with the OS default app.
